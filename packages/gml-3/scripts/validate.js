...
const urlToFs = new UrlMapper({
  ...
  'http://schemas.opengis.net/gml/3.2.1/': '../../spec/gml-3/',
  'http://schemas.opengis.net/iso/19139/20070417/gmd/': '../../spec/gmd/',
  'http://schemas.opengis.net/iso/19139/20070417/gco/': '../../spec/gco/',
  'http://schemas.opengis.net/iso/19139/20070417/gss/': '../../spec/gss/',
  'http://schemas.opengis.net/iso/19139/20070417/gts/': '../../spec/gts/',
  'http://schemas.opengis.net/iso/19139/20070417/gsr/': '../../spec/gsr/',
});
...
async function boot() {
  const [xsd] = await Promise.all([
    promises.readFile('../../spec/gml-3/gml.xsd', 'utf-8'),
  ]);
...
async function main() {
  const [files, validator] = await Promise.all([
    promises
      ...
      .then((files) => files.filter((f) => f.endsWith('.gml'))),
    boot(),
  ]);
